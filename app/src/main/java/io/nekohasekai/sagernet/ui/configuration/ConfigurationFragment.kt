...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!select) {
            ...
            // Add toolbar buttons
            toolbar.menu.add(0, 1, 0, "Developer Info").setIcon(R.drawable.ic_info).setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS)
            toolbar.menu.add(0, 2, 0, "Donation").setIcon(R.drawable.ic_heart).setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS)
                ...
                when (it.itemId) {
                    1 -> {
                        MaterialAlertDialogBuilder(requireContext())
                            ...
                            .setMessage("المطور: تم التطوير بواسطة الرفاعي\nتيليجرام: https://t.me/alrufaaey")
                    ...
                    2 -> {
                        MaterialAlertDialogBuilder(requireContext())
                            .setTitle("تبرع")
                            .setMessage("1403259 بنك الخرطوم\n51713141 فوري")
                            .setPositiveButton("نسخ بنك الخرطوم") { _, _ ->
                                val clipboard = requireContext().getSystemService(CLIPBOARD_SERVICE) as ClipboardManager
                                val clip = ClipData.newPlainText("Bank Info", "1403259")
                                clipboard.setPrimaryClip(clip)
                                Snackbar.make(requireView(), "تم نسخ رقم حساب بنك الخرطوم", Snackbar.LENGTH_SHORT).show()
                            }
                            .setNegativeButton("نسخ فوري") { _, _ ->
                                val clipboard = requireContext().getSystemService(CLIPBOARD_SERVICE) as ClipboardManager
                                val clip = ClipData.newPlainText("Fawry Info", "51713141")
                                clipboard.setPrimaryClip(clip)
                                Snackbar.make(requireView(), "تم نسخ رقم حساب فوري", Snackbar.LENGTH_SHORT).show()
                            }
                            .show()
                        true
                    }
        ...
        when (item.itemId) {
            ...
            R.id.action_import_clipboard -> {
                ...
                if (text.isBlank()) {
                    ...
                    return@action_import_clipboard
